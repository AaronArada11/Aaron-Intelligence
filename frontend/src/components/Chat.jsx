...
function Chat({ onClose }) {
  ...
  return (
    <div className="fixed bottom-24 right-6 z-50 w-[320px] h-[400px] flex flex-col rounded-2xl overflow-hidden bg-gray-950 text-gray-100 shadow-2xl shadow-black/50 border border-gray-800">
      <header className="flex items-center gap-3 px-4 py-3 border-b border-gray-800 bg-gray-900/80">
      ...
      <main className="flex-1 overflow-y-auto px-4 py-4">
        <div className="space-y-4">
          {messages.map((msg, idx) => (
            ...
            >
              {msg.role === 'assistant' && (
                <div className="flex-shrink-0 w-7 h-7 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center shadow-lg shadow-blue-500/20">
              ...
              >
                {msg.role === 'assistant' ? (
                  <div className="prose prose-invert prose-sm max-w-none
                    ...
                    prose-pre:bg-gray-900 prose-pre:border prose-pre:border-gray-700 prose-pre:rounded-lg prose-pre:p-3 prose-pre:text-xs
      ...
      <footer className="border-t border-gray-800 bg-gray-900/80 px-4 py-3">
        {error && (
        ...
        <form onSubmit={handleSubmit} className="relative flex items-center gap-2">
          <input
            ...
            disabled={isLoading}
            className="flex-1 bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 pr-12 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          />
          <button
            type="submit"
            disabled={isLoading || !input.trim()}
            className="absolute right-1.5 top-1/2 -translate-y-1/2 p-2 rounded-lg bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 disabled:text-gray-500 text-white transition-colors disabled:shadow-none"
          >
